...
def compute_optical_flow(prev, curr):
    ...
    flowDTVL1 = cv2.cuda_OpticalFlowDual_TVL1.calc(
                dtvl1, cuMat1, cuMat2, None,
                )
    
    flow_frame = flowDTVL1.download()
    return flow_frame
...
def runrgbflow(sorted_list_frames, dim):
    ...
    for id, full_file_path in enumerate(sorted_list_frames):
        ...
        current_img_bw = cv2.cvtColor(current_img, cv2.COLOR_BGR2GRAY)
        if id == 0:
            prev_img_bw = current_img_bw
            continue
        flow = compute_optical_flow(prev_img_bw, current_img_bw)
        flow = np.clip(flow, -20, 20)
        flow = flow / 20.0
        prev_img_bw = current_img_bw
        
        # Resize
        current_img = cv2.resize(current_img, (img_width,img_height), interpolation=cv2.INTER_LINEAR)
        flow = cv2.resize(flow, (img_width,img_height), interpolation=cv2.INTER_LINEAR)
        # Create bw image
#        current_img_bw = cv2.cvtColor(current_img, cv2.COLOR_BGR2GRAY)
...
#         flow = compute_optical_flow(prev_img_bw, current_img_bw)
        ...
        # Normalise RGB
        current_img = current_img.astype('float64')
        current_img = (current_img / 255.0) * 2 - 1
